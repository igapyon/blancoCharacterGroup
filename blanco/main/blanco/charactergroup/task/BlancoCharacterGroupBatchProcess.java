...
/**
 * バッチ処理クラス [BlancoCharacterGroupBatchProcess]。
 *
 * バッチ処理の呼び出し例。
 * <code>
 * java -classpath (クラスパス) blanco.charactergroup.task.BlancoCharacterGroupBatchProcess -help
 * </code>
 */
public class BlancoCharacterGroupBatchProcess {
    ...
    public static final void main(final String[] args) {
        ...
        for (int index = 0; index < args.length; index++) {
            ...
            if (arg.startsWith("-verbose=")) {
                ...
                System.out.println("BlancoCharacterGroupBatchProcess: 入力パラメータ[" + arg + "]は無視されました。");
                isNeedUsage = true;
            }
        }
        ...
        if( isFieldMetadirProcessed == false) {
            System.out.println("BlancoCharacterGroupBatchProcess: 処理開始失敗。入力パラメータ[input]の必須フィールド値[metadir]に値が設定されていません。");
    ...
    public int process(final BlancoCharacterGroupProcessInput input) throws IOException, IllegalArgumentException {
        ...
        // この箇所でコンパイルエラーが発生する場合、BlancoCharacterGroupProcessインタフェースを実装して blanco.charactergroup.taskパッケージに BlancoCharacterGroupProcessImplクラスを作成することにより解決できる場合があります。
    ...
    public final int execute(final BlancoCharacterGroupProcessInput input) throws IllegalArgumentException {
        try {
            ...
            System.out.println("BlancoCharacterGroupBatchProcess: 入力例外が発生しました。バッチ処理を中断します。:" + ex.toString());
            // 入力異常終了。
            return END_ILLEGAL_ARGUMENT_EXCEPTION;
        } catch (IOException ex) {
            System.out.println("BlancoCharacterGroupBatchProcess: 入出力例外が発生しました。バッチ処理を中断します。:" + ex.toString());
            // 入力異常終了。
            return END_IO_EXCEPTION;
        } catch (RuntimeException ex) {
            System.out.println("BlancoCharacterGroupBatchProcess: ランタイム例外が発生しました。バッチ処理を中断します。:" + ex.toString());
            ex.printStackTrace();
            // 異常終了。
            return END_ERROR;
        } catch (Error er) {
            System.out.println("BlancoCharacterGroupBatchProcess: ランタイムエラーが発生しました。バッチ処理を中断します。:" + er.toString());
            er.printStackTrace();
            // 異常終了。
            return END_ERROR;
        }
    ...
    public static final void usage() {
        ...
        System.out.println("  java blanco.charactergroup.task.BlancoCharacterGroupBatchProcess -verbose=値1 -metadir=値2 -targetdir=値3 -tmpdir=値4 -encoding=値5 -targetlang=値6");
        ...
        System.out.println("      説明[ターゲットとなるプログラミング言語処理系名。java, cs, js, vb, phpが選択可能]");
        System.out.println("      型[文字列]");
    ...
    public void validateInput(final BlancoCharacterGroupProcessInput input) throws IllegalArgumentException {
        ...
        if (input.getMetadir() == null) {
            throw new IllegalArgumentException("BlancoCharacterGroupBatchProcess: 処理開始失敗。入力パラメータ[input]の必須フィールド値[metadir]に値が設定されていません。");
        }
    }
}
